...
if ( 'update' === $action ) { // We are saving settings sent from a settings page.
	...
	// Kiểm tra xem URL trước đó có tồn tại
	if ( ! empty( $referer ) ) {
		// Phân tích URL trước đó
		$parsed_url = parse_url( $referer );

		// Kiểm tra xem có query string trong URL không
		if ( isset( $parsed_url['query'] ) ) {
			// Thêm tham số truy vấn 'settings-updated=true' vào query string
			$updated_url = add_query_arg( 'settings-updated', 'true', $parsed_url['scheme'] . '://' . $parsed_url['host'] . $parsed_url['path'] . '?' . $parsed_url['query'] );
		} else {
			// Thêm tham số truy vấn 'settings-updated=true' vào URL
			$updated_url = add_query_arg( 'settings-updated', 'true', $parsed_url['scheme'] . '://' . $parsed_url['host'] . $parsed_url['path'] );
		}

		// Sử dụng wp_safe_redirect() để duy trì giao thức HTTPS và chuyển hướng đến URL đã cập nhật
